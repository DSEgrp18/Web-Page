...
| Document extraction: PDF, DOCX, images | Working. 97.4% of a real 168-page Grade 11 textbook readable; what is not is flagged, not narrated. FM-Abhaya legacy text is decoded; DOCX gives paragraph text but not Word's page layout; a standalone image is one page and depends on OCR being configured |
| Reader API: upload, pages, segments, audio, progress, bookmarks | Working, and accepts PDF, DOCX, PNG and JPEG. Accounts, PostgreSQL and a Celery queue are implemented and selected by configuration. [Compose](infra/README.md) runs PostgreSQL and Celery; the code's own **defaults** are still a trusted header, in-memory storage and a thread per job. Audio lives in the database rather than object storage |
| Reader interface: library, two-panel reader, player, bookmarks, chapters | Working. **No testing with assistive technology has been done**, which CLAUDE.md treats as a release blocker, and no screen reader has ever been run against any of it. Chapter navigation and multi-file upload landed; the reading settings screen, the voice-loading notice, the designed error states and offline downloads are still open issues |
| Sinhala interface text | **Awaiting native-speaker review** ([one file](apps/web/src/lib/strings.ts)) |
| Scanned and broken pages: Sinhala OCR | Working locally with Tesseract (`sin`), off by default and `broken` under Compose. It also reads uploaded PNG and JPEG pages. Every OCR'd page is marked for review; there is **no review or correction workflow yet**, and OCR accuracy has not been measured |
